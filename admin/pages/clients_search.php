...
if (!isset($_GET['term'])) {
    echo '<script>location.href="' . APP_URL . '/admin/clients";</script>';
    exit;
  }
...
$list = $clients->searchClients($term, $inicio, $itens_por_pagina);
$count_t = $clients->listAllSearch($term);
...
<body class="g-sidenav-show  bg-gray-200">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <div class="container-fluid py-4">
                  ...
                  <nav aria-label="Navegação de página exemplo">
                    <ul class="pagination">
                      ...
                      if ($pagina_atual > 1 && $pagina_atual >= ($MAX_ITEMS - 1)) {
                        $paginationHTML[] = '<li class="page-item"><a href="' . $next_link . '1'.$extra_link.' " class="page-link"><i class="fa fa-angle-double-left"></i></a></li>';
                      }

                      // Before page
                      $beforePage = $pagina_atual > 1 ? ($pagina_atual - 1) : false;
                      if ($beforePage) {
                        $paginationHTML[] = '<li class="page-item"><a href="' . $next_link . $beforePage . ''.$extra_link.'" class="page-link"><i class="fa fa-angle-left"></i></a></li>';
                      } else {
                      ...
                      for ($i = 0; $i < min($MAX_ITEMS, $total_paginas); $i++) {
                        ...
                        if ($page <= $total_paginas) {
                          if ($page == $pagina_atual) {
                            $paginationHTML[] = '<li class="page-item active"><a href="' . $next_link . $page . ''.$extra_link.'" class="page-link">' . $page . '</a></li>';
                          } else {
                            $paginationHTML[] = '<li class="page-item"><a href="' . $next_link . $page . ''.$extra_link.'" class="page-link">' . $page . '</a></li>';
                          }
                        }
                      ...
                      if ($endPage == $pagina_atual) {
                        ...
                        $paginationHTML[] = '<li class="page-item"><a href="' . $next_link . $nextPage . ''.$extra_link.'" class="page-link"><i class="fa fa-angle-right"></i></a></li>';
                      }

                      // Last page
                      if (!in_array($total_paginas, $pagesListed)) {
                        $paginationHTML[] = '<li class="page-item"><a href="' . $next_link . $total_paginas . ''.$extra_link.'" class="page-link"><i class="fa fa-angle-double-right"></i></a></li>';
                      }
      ...
      <script type="text/javascript" src="assets/js/client.js?v=3"></script>
